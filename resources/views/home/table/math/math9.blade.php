...
<style>
    ...
    <div class="flex flex-col lg:flex-row items-center justify-between m-2">
        <div class="flex items-center mb-2 lg:mb-0">
            <form id="resetearForm" action="{{ route('reset.math9') }}" method="POST">
                @csrf
            ...
            <div class="m-2">
                <a href="{{route('exportar.excel.math9')}}" class="bg-green-700 hover:bg-green-800 text-white font-bold py-2 px-4 rounded-lg">
        ...
        <div class="relative">
            <form action="{{route('table.math9')}}" method="GET" class="flex items-center">
                    ...
                    <tbody class="bg-white">
                        @foreach ($promedioData as $data)
                            <tr>
                                ...
                                <td >
                                    <button
                                        onclick="window.location.href='{{ route('answer.mathNoveno', ['userId' => $data['user']->id]) }}'"
                                        class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-1 px-2 rounded transition duration-300 ml-2">
                                        <i class="fas fa-file text-sm"></i>
                                    </button>
                                </td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <a href="{{ route('history9.user', $data['user']->id ) }}" class="text-blue-500 hover:underline">
                                        Ver historial
                                    </a>
                                </td>
                            </tr>
